...
def printHelp() {
  log.info"""
  Usage:
    nextflow run ncov-nf/nextstrain -profile (singularity,docker,conda) -mode (refine_iterations, random_subsets, lineages)
  ...
  Other Workflow parameters:
      ...
      --master_fasta	      		Path to the master (unaligned) multi-FASTA of PHO sequences to create sub-sampled builds from. Requires make_alignment = true

      --nextalign             		Path to a master multi-alignment file generated by Nextclade. Used if an alignment step is not used for the builds

      --cache                 		If set as '', specify the cache directory in the work directory

      --tracedir              		Path specifying the output for traces such as the log, execution trace, pipeline dag, and timeline report
      ...
      --make_alignment	        	Specify if MAFFT should be used to align non-alignment sequences, or not set to false, use the pre-existing nextalign file
